...
inline static uintptr_t virt_to_phys(void* virt) {
...
inline uint32_t pkt_buf_alloc_batch(struct mempool* mempool, struct pkt_buf* bufs[], uint32_t num_bufs) {
...
inline struct pkt_buf* pkt_buf_alloc(struct mempool* mempool) {
	struct pkt_buf* buf = NULL;
	pkt_buf_alloc_batch(mempool, &buf, 1);
	return buf;
}

inline void pkt_buf_free(struct pkt_buf* buf) {
	if (unlikely(buf->ref_count <= 0)) {
		error("buf ref counter smaller than 0 ");
	}
	buf->ref_count --;
	if(buf->ref_count != 0)
		return;
